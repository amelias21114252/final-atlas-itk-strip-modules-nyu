...
# ============================================================
# python plot_multi_IV_final_UCSC_Category_A_Di_warning.py
#
# Fast IV analysis + warning / Category A / D(i) summary generator.
# ONLY saves plots for yellow-warning, Category A, and Category D(i) modules.
#
...
#
# Category E(i)-only modules are still analyzed and included in summaries,
# but no PDF/PNG is saved for them.
#
# Run:
#   python plot_multi_IV_final_UCSC_Category_A_Di_warning.py  -i "UCSC/ML/*/*.json" -o "UCSC/ML" --workers 6
#
# Optional faster test without PNG:
#   python plot_multi_IV_final_UCSC_Category_A_Di_warning.py \
#       -i "UCSC/ML/*/*.json" -o "UCSC/ML" --workers 6 --no_png
#
# Saves warning / Category A / D(i) plots in each module folder and copies all selected PDFs into:
#   UCSC/ML/category_warning_A_Di_IV_plots_pdf/SN20USBML....pdf
#
...
# ============================================================
# Worker: analyze one module and plot warning, Category A, or Category D(i)
# ============================================================
...
def plot_one_module_worker(args_tuple):
    ...
    # Plot the requested IV warning/problem groups:
    #   Yellow warning = current above 300 nA and at or below 600 nA
    #   Category A = current above 600 nA
    #   Category D(i) = incomplete IV dataset
    # Category E(i)-only modules are not plotted.
    ...
    if not result["is_selected_plot_module"]:
        ...
        print(f"Skipping plot (not warning, Category A, or D(i)): {module_sn}")
        return result
    ...
    print(f"Saving warning / Category A / D(i) plot as {save_pdf}")
    plt.savefig(save_pdf, format="pdf")

    if save_png:
        print(f"Saving warning / Category A / D(i) plot as {save_png_path}")
        plt.savefig(save_png_path, format="png", dpi=300)

    plt.close(fig)

    # Also copy every selected warning / Category A / D(i) PDF into one shared folder.
    selected_pdf_dir = Path(output_base_dir) / "category_warning_A_Di_IV_plots_pdf"
    ...
    print(f"Copied warning / Category A / D(i) PDF to {selected_pdf_copy}")
...
def write_error_summary(results, output_file):
    ...
    with output_file.open("w") as f:
        ...
        f.write("Plot mode: yellow-warning, Category A, and Category D(i) modules\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Fast UCSC IV analyzer and warning / Category A / D(i) summary generator. "
            "Yellow-warning, Category A, and Category D(i) plots are saved."
        )
    )
    parser.add_argument("-i", "--input", required=True, help='Input glob, for example "UCSC/ML/*/*.json"')
    ...
    print("\n" + "=" * 80)
    print("FAST UCSC IV ANALYSIS — PLOT WARNING, CATEGORY A, AND D(i)")
    print("=" * 80)
    ...
    if args.workers <= 1:
        for index, task in enumerate(tasks, start=1):
            ...
            status = "PLOTTED WARNING/A/D(i)" if result["plot_pdf"] else "NOT WARNING/A/D(i)—SKIPPED"
    ...
    print(f"Warning / Category A / D(i) plots saved: {selected_plots}")
    print(f"Warning / Category A / D(i) PDFs copied to shared folder: {selected_pdf_copies}")
    print(f"Non-selected module plots skipped: {not_selected_skipped}")
    print(f"Warning / Warning / Category A / D(i) plots saved under: {output_base_dir}/SN20USBML.../IV/")
    print(f"Shared warning / Category A / D(i) PDF folder: {output_base_dir}/category_warning_A_Di_IV_plots_pdf/")
    print(f"Saved: {iv_error_summary_file}")
    print(f"Saved: {iv_category_summary_file}")
    print("")
    print("Important:")
    print("  Plot saved only = yellow warning, Category A, or Category D(i)")
