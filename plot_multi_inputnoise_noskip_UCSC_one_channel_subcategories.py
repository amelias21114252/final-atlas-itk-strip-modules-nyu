#!/usr/bin/env python3
"""
plot_multi_inputnoise_noskip_UCSC_one_channel_subcategories.py

Analyze UCSC no-skip input-noise JSON files and save plots only for problem
module/stream combinations.

Run:
    python plot_multi_inputnoise_noskip_UCSC_one_channel_subcategories.py -i "UCSC/HX/SN*/*.json" -o "UCSC/HX2"

Optional PDF-only run:
    python plot_multi_inputnoise_noskip_UCSC_one_channel_subcategories.py \
        -i "UCSC/HX/SN*/*.json" \
        -o "UCSC/HX2" \
        --no_png
...
Plot behavior:
    ...
    * Each PDF is copied into every matching subcategory folder under:
          UCSC/HX/problem_inputnoise_noskip_plots/
...
def write_error_summary_txt(results, output_dir, unreadable_files):
    ...
    summary_path = output_dir / "inputnoise_noskip_error_summary_ucsc.txt"
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("UCSC INPUT NOISE NOSKIP ERROR SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def write_category_summary_txt(results, output_dir):
    ...
    summary_path = output_dir / "inputnoise_noskip_category_summary_ucsc.txt"
    comments = build_module_comments(results)

    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("UCSC INPUT NOISE NOSKIP CATEGORY SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze both UCSC no-skip input-noise streams and save only the "
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Glob pattern, for example 'UCSC/HX/SN*/*.json'",
    )
    ...
    if args.input:
        ...
        if not input_files:
            input_files = sorted(
                glob(f"UCSC/HX/SN{serial}/SN{serial}_*.json")
            )
    ...
    print("\n" + "=" * 80)
    print("UCSC INPUT NOISE NOSKIP — ONE-CHANNEL PLOT TRIGGER")
    print("=" * 80)
