#!/usr/bin/env python3
"""
plot_multi_inputnoise_UCSC_one_channel_subcategories.py

Analyze UCSC input-noise JSON files and save plots only for problem
module/stream combinations.

Run:
    python plot_multi_inputnoise_UCSC_one_channel_subcategories.py -i "UCSC/HX/SN*/*.json" -o "UCSC/HX"

Optional PDF-only run:
    python plot_multi_inputnoise_UCSC_one_channel_subcategories.py \
        -i "UCSC/HX/SN*/*.json" \
        -o "UCSC/HX" \
        --no_png
...
Plot behavior:
    ...
    * Each PDF is copied into every matching subcategory folder under:
          UCSC/HX/problem_inputnoise_plots/
...
def write_error_summary_txt(results, output_dir, unreadable_files):
    ...
    summary_path = output_dir / "inputnoise_error_summary_ucsc.txt"
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("UCSC INPUT NOISE ERROR SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def write_category_summary_txt(results, output_dir):
    ...
    summary_path = output_dir / "inputnoise_category_summary_ucsc.txt"
    comments = build_module_comments(results)

    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("UCSC INPUT NOISE CATEGORY SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze both UCSC input-noise streams and save only the "
    ...
    print("\n" + "=" * 80)
    print("UCSC INPUT NOISE — ONE-CHANNEL PLOT TRIGGER")
    print("=" * 80)
