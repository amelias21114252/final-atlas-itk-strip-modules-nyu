...
Run:
    python plot_multi_inputnoise_BNL_one_channel_subcategories.py -i "BNL/HX/SN*/*.json" -o "BNL/HX2"

Optional PDF-only run:
    python plot_multi_inputnoise_BNL_one_channel_subcategories.py \
        -i "BNL/HX/SN*/*.json" \
        -o "BNL/HX2" \
        --no_png
...
Plot behavior:
    ...
    * Each PDF is copied into every matching subcategory folder under:
          BNL/HX/problem_inputnoise_plots/
...
def write_error_summary_txt(results, output_dir, unreadable_files):
    ...
    summary_path = output_dir / "inputnoise_error_summary_bnl.txt"
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL INPUT NOISE ERROR SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def write_category_summary_txt(results, output_dir):
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL INPUT NOISE CATEGORY SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze both BNL input-noise streams and save only the "
    ...
    print("\n" + "=" * 80)
    print("BNL INPUT NOISE — ONE-CHANNEL PLOT TRIGGER")
    print("=" * 80)
