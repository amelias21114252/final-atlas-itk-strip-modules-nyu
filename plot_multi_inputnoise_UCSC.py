...
Usage:
python plot_multi_inputnoise_UCSC.py -i 'UCSC/HX/SN20USBHX2002099/SN20USBHX2002099_*.json'
...
def filter_input_files(infiles, keep_fit_code=4):
    ...
    for f in infiles:
        try:
            ...
            module_name = safe_get_module_name(f) or "Unknown"
            ...
            add_error("D", "Unknown", f, f"File skipped or unreadable: {e}")
...
def mk_inoise_plot(module_name, input_files, stream="under", output_base_dir="UCSC/HX3"):
    ...
    for idx, f in enumerate(files):
        try:
            ...
            # Category B
            if mean_val > 1000:
                msg = f"{basename} — mean input noise = {mean_val:.1f} > 1000"
            ...
            # Do not plot very large mean values
            if mean_val > 1000:
    ...
    save_dir = os.path.join(output_base_dir, module_name, "inputnoise")
    mkdir(save_dir)
...
def write_error_summary_txt(output_dir):
    mkdir(output_dir)

    summary_path = os.path.join(output_dir, "inputnoise_error_summary_ucsc.txt")

    with open(summary_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("INPUT NOISE ERROR SUMMARY\n")
        f.write("=" * 80 + "\n\n")

        categories = {
            "B": "CATEGORY B — input noise mean greater than 1000",
    ...
    print("\n" + "=" * 80)
    print(f"Saved error summary TXT file:")
    print(summary_path)
    print("=" * 80)
...
def main():
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Glob pattern for JSON files, e.g. 'UCSC/HX/SN*/*.json'",
    )
    ...
    parser.add_argument(
        ...
        "--output",
        default="UCSC/HX3",
        help="Output directory for saved plots and TXT summary",
    )
    ...
    if args.input:
        ...
        pattern = f"SN{serial}/SN{serial}_*.json"
    ...
    if len(input_files) == 0:
        print("❌ CATEGORY E: No JSON files found. Check your path or glob pattern.")
        add_error("E", "Unknown", None, "No JSON files found. Script did not run for any module.")
        write_error_summary_txt(args.output)
        return
