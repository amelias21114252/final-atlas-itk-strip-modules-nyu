...
# Combined BNL, LBNL, and UCSC timestamp lookup.
# Run all sites:
#   export ITK_DB_AUTH=YOUR_TOKEN
#   python get_test_timestamp_full_list_all_sites.py
# Run selected sites:
#   python get_test_timestamp_full_list_all_sites.py --sites BNL UCSC
...
def save_txt(results, output_txt, test_name, institute):
    output_txt = Path(output_txt)

    with output_txt.open("w") as f:
        f.write("=" * 80 + "\n")
        f.write(f"{institute} TIMESTAMP LIST\n")
        f.write("=" * 80 + "\n\n")
...
def run_site(institute, config, pairs_csv, test_name, max_workers):
    hx_file = config["hx_file"]
    ml_file = config["ml_file"]
    output_json = config["output_json"]
    output_txt = config["output_txt"]

    hx_serials = read_serial_file(hx_file, serial_type="HX")
    ml_serials = read_serial_file(ml_file, serial_type="ML")
    ...
    print("\n" + "=" * 80)
    print(f"{institute} TIMESTAMP LOOKUP")
    print("=" * 80)
    print(f"HX file: {hx_file}")
    print(f"ML file: {ml_file}")
    ...
    results = [results_by_index[index] for index in range(len(hx_serials))]
    ...
    save_txt(results, output_txt, test_name, institute)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Get Response Curve TC timestamps for BNL, LBNL, and UCSC "
            "and pair HX serials with ML serials."
        )
    )
    parser.add_argument(
        "--sites",
        nargs="+",
        choices=["BNL", "LBNL", "UCSC"],
        default=["BNL", "LBNL", "UCSC"],
        help="Institutes to process; default: all three",
    )
    parser.add_argument(
        "--pairs_csv",
        default="serial_lists/all_module_hybrid_pairs.csv",
        help="CSV file containing ML-HX pairs",
    )
    parser.add_argument(
        "--test_name",
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Parallel workers used per institute",
    )
    ...
    site_configs = {
        "BNL": {
            "hx_file": "serial_lists/bnl_hx_serials.txt",
            "ml_file": "serial_lists/bnl_ml_serials.txt",
            "output_json": "timestamps_list_bnl.json",
            "output_txt": "formatted_timestamps_bnl.txt",
        },
        "LBNL": {
            "hx_file": "serial_lists/lbnl_hx_serials.txt",
            "ml_file": "serial_lists/lbnl_ml_serials.txt",
            "output_json": "timestamps_list_lbnl.json",
            "output_txt": "formatted_timestamps_lbnl.txt",
        },
        "UCSC": {
            "hx_file": "serial_lists/ucsc_hx_serials.txt",
            "ml_file": "serial_lists/ucsc_ml_serials.txt",
            "output_json": "timestamps_list_ucsc.json",
            "output_txt": "formatted_timestamps_ucsc.txt",
        },
    }

    failures = []
    for institute in args.sites:
        try:
            run_site(
                institute=institute,
                config=site_configs[institute],
                pairs_csv=args.pairs_csv,
                test_name=args.test_name,
                max_workers=args.max_workers,
            )
        except Exception as error:
            failures.append((institute, str(error)))
            print(f"\n❌ {institute} failed: {error}")

    print("\n" + "=" * 80)
    print("ALL REQUESTED INSTITUTES FINISHED")
    print("=" * 80)

    if failures:
        for institute, error in failures:
            print(f"❌ {institute}: {error}")
        raise SystemExit(1)
